...
int shortest_path_bfs(const vector<int>& base_dist,const string &flat_grid, const RouteRequest &request,
    ...
    //const int d_index[4] = {-cols, cols, -1, 1};
    while (frontier_head < frontier_tail) {
        int current_index = frontier[frontier_head++];
        if (current_index == goal_index) {
            return distance[current_index];
        }
        //int neighbours[4]={current_index - cols, current_index + cols, current_index - 1, current_index + 1};
        // for(int next_index: neighbours){
        //     if (flat_grid[next_index] == '#') {
        //         continue;
        //     }
 ...
 */
RunSummary run_all_requests(const string &flat_grid,
                            const vector<RouteRequest> &requests,
                            vector<int> &heatmap,int cols) {
    RunSummary summary;
...
int main(int argc, char **argv) {
    ...
    RunSummary summary = run_all_requests(flat_grid, requests, heatmap,kCols);
