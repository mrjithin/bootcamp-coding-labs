...
// with a *different* function like fclose, sqlite3_close,
// pthread_mutex_destroy, or close().
//
...
// of `delete`. By substituting your own, you can wrap any C-style
// acquire/release API in RAII. This is the bridge from "smart pointers" to
// "RAII as a general technique."
//
...
//
// Uncomment each part's main() block when you reach it. Comment out the
// previous.
//
// Compile with:
//   g++ -std=c++20 -Wall -Wextra -fsanitize=address,undefined -g
//   custom_deleter.cpp -o custom_deleter
// =============================================================================
...
//      ANS: decltype gives the lambda's actual type at compile time. auto
//      cannot be used here since here we need to specify the type and it is not
//      used in the context of variable declaration for auto to work.
...
//      ANS: We need to pass the custom deleter to unique_ptr so that it knows
//      how to deallocate the resource.
...
//    ANS: It should be same (8 bytes) since the lambda does not capture any
//    state and an empty functor occupies zero space due to compiler
//    optimizations.
// =============================================================================

// TODO Part 1.1: define the lambda. It should be a non-capturing lambda that
//                accepts a FILE* and calls std::fclose if the pointer is
//                non-null. Log something like "fclose called on " + the FILE*
//                address to cerr.
auto closer = [](FILE* f) {
    if (f) {
        std::fclose(f);
...
//     // empty-base-optimization so the deleter takes 0 bytes inside the
//     // unique_ptr.
...
//
// Your tasks:
//   1. Define struct CountingFileCloser with an int* member and
//   operator()(FILE*).
//      QUESTION: why an int* rather than an int? (Hint: we want multiple
//                unique_ptrs to update the SAME counter.)
//   ANS: We want all unique_ptr to update the same counter instead of having a
//   separate counter. If we used an int, each counter would be independent.
//   2. Create a using-alias CountedFilePtr = std::unique_ptr<FILE,
//   CountingFileCloser>.
//   3. Write open_counted(path, mode, counter) — a factory that takes the
//   counter
...
//      PREDICT before running: how does it compare to sizeof(FilePtr) from Part
//      1?
//    ANS: It should be double (16 bytes) since the struct has an additional
//    int* member.
// =============================================================================
...
// TODO Part 2.3: implement the factory. The deleter instance must be
// constructed
...
// =============================================================================
// EXPECTED OUTPUT (after both parts are uncommented and the TODOs are filled
// in)
//
// Part 1:
...
//
//   sizeof(FILE*)          = 8
//   sizeof(CountedFilePtr) = 16                  <-- doubled by the stateful
//   deleter
//
...
//
// The closest Swift analogue: wrapping a C resource (CFType,
// UnsafeMutablePointer, or any pointer from a C API) inside a Swift `class` and
// releasing it in `deinit`. Swift FORCES you to write the wrapper class. C++
// lets you bolt the cleanup onto a smart pointer directly with no class —
// that's the win custom deleters provide.
//
...
//
// In both languages, the cost story is identical: stateful cleanup costs
// storage.
// =============================================================================
